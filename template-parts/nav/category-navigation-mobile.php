	...
	$categories = get_terms(
		...
		<div class="row categories-row ptb-2">
			...
			<?php foreach ($categories as $maincategori => $mcv): ?>
					...
					<?php $sub_cat_link = get_bloginfo('url') . '/sub-category-nav' . $lang . '?id=' . $term_id; ?>

					<?php if ($count_items_in_subcat != 0 ): ?>
						<div class="col-6 category-button-holder">
						<a class="btn btn-sk btn-cat mtb-1" href="<?php echo $sub_cat_link; ?>" data-category="<?php echo $term_id; ?>" style="background-color: <?php echo $bg_color; ?>; border-color:<?php echo $bg_color; ?> " aria-label="Categories">
